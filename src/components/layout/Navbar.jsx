...
function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-dark-900/90 backdrop-blur-md border-b border-dark-700">
      ...
      >
        <Link to="/parches" className="font-display font-extrabold text-xl text-white hover:text-brand-400 transition-colors">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 text-sm text-gray-400 hover:text-red-400 transition-colors px-3 py-2 rounded-xl hover:bg-dark-700"
          >
            <FiLogOut size={16} />
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-dark-900 border-t border-dark-700 flex justify-around py-2 z-50">
        {navLinks.map(({ to, label, icon: Icon }) => {
          const isActive = location.pathname === to
          return (
            <Link
